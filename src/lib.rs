...
//! cube maps, volumes, buffers, and arrays of the above. It is used by
//! DirectX, OpenGL, Vulkan, and other graphics APIs.
//!
//! A DDS file consists of two parts: a header and a data section. The header
//! describes the type of data in the file (e.g. a BC1-compressed 100×200px
//! texture) and determines the layout of the data section. The data section
//! then contains the binary pixel data.
//!
//! # Features
//!
...
//!   [`Decoder::read_surface`]. Use [`Decoder::skip_mipmaps`] to skip any
...
//!   arrays.
//! - Volumes must be decoded one depth slice at a time using
...
//!
//! A single texture can be created as follows:
//!
//! ```no_run
//! # use dds::*;
//! fn save_rgba_image(
...
//!
//! Mipmaps are automatically generated by default. Options for mipmap
//! generation can be configured using [`Encoder::mipmaps`].
//!
//! Most formats also support encoding options to modify how image data is
//! encoded. These can be set using [`Encoder::options`]. For example, most
//! formats support [dithering](EncodeOptions::dithering), and compressed
...
//! ## Progress reporting
//!
//! The decoder is generally so fast that progress reporting is not needed when
...
//! to receive periodic updates on the encoding progress. See the [`Progress`]
...
//! API for encoding and decoding DDS surfaces. It is generally not necessary to
//! use this API.
//!
