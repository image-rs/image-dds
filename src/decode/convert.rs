...
#[cfg(test)]
mod test {
    macro_rules! test_to_unorm8 {
        ($name:ident, $convert:path, $max_in:expr) => {
            #[test]
            fn $name() {
                assert_eq!($convert(0), 0);
                assert_eq!($convert($max_in), 255);

                for x in 0..=$max_in {
                    let expected = (x as f64 * 255.0 / $max_in as f64).round() as u8;
                    assert_eq!($convert(x), expected);
                }
            }
        };
    }
    test_to_unorm8!(n1_to_n8, super::n1::n8, 1);
    test_to_unorm8!(n2_to_n8, super::n2::n8, 3);
    test_to_unorm8!(n4_to_n8, super::n4::n8, 15);
    ...
    macro_rules! test_to_unorm16 {
        ($name:ident, $convert:path, $max_in:expr) => {
            ...
            fn $name() {
                assert_eq!($convert(0), 0);
                assert_eq!($convert($max_in), 65535);

                for x in 0..=$max_in {
                    let expected = (x as f64 * 65535.0 / $max_in as f64).round() as u16;
                    assert_eq!($convert(x), expected);
                }
            }
        };
    }
    test_to_unorm16!(n1_to_n16, super::n1::n16, 1);
    test_to_unorm16!(n2_to_n16, super::n2::n16, 3);
    ...
    #[test]
    fn s8_to_n8() {
        assert_eq!(super::s8::n8(i8::MIN as u8), 0);
        assert_eq!(super::s8::n8(-i8::MAX as u8), 0);
        assert_eq!(super::s8::n8(0_i8 as u8), 128);
        assert_eq!(super::s8::n8(i8::MAX as u8), 255);

        for x in 0..=u8::MAX {
            let xi = x as i8;
            let expected =
                ((xi.max(-i8::MAX) as f64 / i8::MAX as f64 + 1.0) / 2.0 * 255.0).round() as u8;
            assert_eq!(super::s8::n8(x), expected);
        }
    }
    #[test]
    fn s8_to_n16() {
        assert_eq!(super::s8::n16(i8::MIN as u8), 0);
        assert_eq!(super::s8::n16(-i8::MAX as u8), 0);
        assert_eq!(super::s8::n16(0_i8 as u8), 32768);
        assert_eq!(super::s8::n16(i8::MAX as u8), 65535);

        for x in 0..=u8::MAX {
            let xi = x as i8;
            let expected =
                ((xi.max(-i8::MAX) as f64 / i8::MAX as f64 + 1.0) / 2.0 * 65535.0).round() as u16;
            assert_eq!(super::s8::n16(x), expected);
        }
    }

    #[test]
    fn s16_to_n8() {
        assert_eq!(super::s16::n8(i16::MIN as u16), 0);
        assert_eq!(super::s16::n8(-i16::MAX as u16), 0);
        assert_eq!(super::s16::n8(0_i16 as u16), 128);
        assert_eq!(super::s16::n8(i16::MAX as u16), 255);

        for x in 0..=u16::MAX {
            let xi = x as i16;
            let expected =
                ((xi.max(-i16::MAX) as f64 / i16::MAX as f64 + 1.0) / 2.0 * 255.0).round() as u8;
            assert_eq!(super::s16::n8(x), expected);
        }
    }
    #[test]
    fn s16_to_n16() {
        assert_eq!(super::s16::n16(i16::MIN as u16), 0);
        assert_eq!(super::s16::n16(-i16::MAX as u16), 0);
        assert_eq!(super::s16::n16(0_i16 as u16), 32768);
        assert_eq!(super::s16::n16(i16::MAX as u16), 65535);

        for x in 0..=u16::MAX {
            let xi = x as i16;
            let expected =
                ((xi.max(-i16::MAX) as f64 / i16::MAX as f64 + 1.0) / 2.0 * 65535.0).round() as u16;
            assert_eq!(super::s16::n16(x), expected);
        }
    }
}
